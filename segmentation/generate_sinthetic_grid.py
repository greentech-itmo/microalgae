...
from segmentation.texture_filter import add_noise, texture, blank_image
...
def generate_dataset(n_samples=3):
    ...
    for im in range(ds.shape[0]):
        ...
        ds[im] = (image+noise)/(image+noise).max()*255
    ...
    np.save('synthetic_dataset/images100.npy', ds)
    np.save('synthetic_dataset/grid_mask100.npy', ds_target)

generate_dataset(n_samples=100)
