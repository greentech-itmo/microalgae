...
Train and validation on real marked data (very small dataset, tent to zero prediction)
"""
...
features_val = torch.tensor(features_val[4:], dtype=torch.float32)
target_val = torch.tensor(target_val[4:], dtype=torch.float32)

features_train = torch.tensor(features_val[:4], dtype=torch.float32)
target_train = torch.tensor(target_val[:4], dtype=torch.float32)
